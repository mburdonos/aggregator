...
class EventService(DbPostgres):
    ...
    async def get_event_by_id(self, id: int) -> Optional[EventApi]:
        data_doc = await self.pg_connection.execute(select(Event.id, Event.coefficient,
                                                           Event.deadline, Event.state_id).where(Event.id == id))
        data = data_doc.first()
        if data:
    ...
    async def update_event(self, val, id: int,  key: str):
        await self.pg_connection.execute(update(Event).where(Event.id == id).values({key: val}))
    ...
    async def get_events(self) -> List[Event]:
        ...
        data = data_doc.all()
        if data:
            return [Event(**event._mapping) for event in data]
        return []
